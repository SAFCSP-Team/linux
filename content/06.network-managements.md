...
Linux provides several commands to query DNS information and interact with web servers.

 ### DNS Lookup
`host` is a simple command to perform DNS lookups, translating domain names to IP addresses and vice versa.
```
host google.com
```
**Sample Output:**
...
````
- Shows the IP address for a domain, along with any available IPv6 addresses and mail servers.
...
> [!NOTE]
> Reverse DNS results for large companies like Google can change frequently, and you may see different pointer records at different times or from different locations.
...
- **Non-authoritative answer**: The DNS server is not the source of this data, but it’s a valid answer.
- **Name**: The domain queried.
- **Address**: The IP address(es) returned for the domain (IPv4 and IPv6).
...
- **Authoritative answers can be found from**: Lists the real IP addresses (IPv4 and IPv6) for the mail server smtp.google.com.
  - **internet address**: IPv4 addresses for smtp.google.com.
  - **has AAAA address**: IPv6 addresses for smtp.google.com.

>[!IMPORTANT]
> Press **Ctrl+C** to quit.

### Detailed DNS query
`dig` (Domain Information Groper) is a flexible and detailed DNS query.
```
dig google.com
```
...
**Find only the answer (just the IP):**
```
