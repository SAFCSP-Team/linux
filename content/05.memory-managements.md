...
An up-to-date virtual file created by the kernel provides information on the system’s memory usage, including a list of memory statistics, such as RAM, swap, buffers, caches, and kernel allocations. 

Each line is a key-value pair format: 
```
Key:    Value   Unit
```
**Sample Output (Top Lines):**
...
- **u**: Display the process’s user/owner, along with more detailed info.
- **x**: Show processes not attached to a terminal.
- **--sort**: Sorts the output by a given field.
- **-%mem**: Shows memory usage percentage. The minus sign means highest first.
- **|**: Takes the output of the command on the left and sends it as input to the command on the right.
- **head**: By default, displays only the first 10 lines of input it receives.
...
>[!NOTE]
> `[anon]` Indicates that the memory region does not belong to a named file or object within the filesystem (memory region is anonymous).

**To provides additional details** such as the resident memory size (RSS), shared memory size, private memory size, and more.
you can use the `-x`option to the extended format 
```
pmap -x 2226
```
...
- **Kbytes**: Total size of the memory region (in KB).
- **RSS**(Resident Set Size): actual RAM the process is using (in KB).
- **Dirty**: Amount of memory in this region that has been modified and not written to disk (KB).
- **Mode**:	Access permissions:
  ...
  - **s** = shared
- **Mapping**: File or object this memory region is mapped to.
